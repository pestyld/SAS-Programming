proc cas;
    source ds;
        data casuser.loans_raw(drop=Date initialCurrentCategories CurrentCategories ) /*accounts*/
             casuser.customers(keep=CurrentCategories ID Salary Date Age EmpLength); /*customer info */
            call streaminit(99);
...
* Save the loans_raw in-memory table as a data source file *;
	table.save / 
		table={name='loans_raw', caslib='casuser'},
		name='loans_raw.sashdat', caslib='casuser',
		replace=TRUE;

* View in-memory table *;
	table.tableInfo / caslib = 'casuser';

* Drop the in-memory table *;
	table.dropTable / name='loans_raw', caslib='casuser', quiet=TRUE;
quit;
...
* Summary Statistics *;
proc cas;
    table.fetch / table=tbl, sortBy={name="ID"}, to=75;
    simple.numRows / table=tbl;
    table.columnInfo / table=tbl;
    simple.distinct / table=tbl;
    simple.freq / table=tbl, inputs={"SalaryGroup", "Category","Year",'CancelledReason'};
    simple.summary / table="loans";
quit;
proc means data=casuser.loans_raw;
    var Amount;
run;
*/