...
data casuser.AppRatings / sessref=conn;
  call streaminit(100);
...
proc cas;
	table.save / 
		table={name='AppRatings', caslib='casuser'},
		caslib='casuser', name='appRatings.sashdat', replace=TRUE;

	table.tableInfo / caslib='casuser';

	table.dropTable / name='AppRatings', caslib='casuser', quiet=TRUE;
quit;
...
* Validate Year Freq     *;
**************************;
proc sgplot data=casuser.apprating;
	vbar Date;
	format Date year.;
run;
...
**************************;
* Validate Year + Server *;
**************************;
proc sgplot data=casuser.apprating;
	vbar date / group=Server;
  format date year.;
run;
...
* Validate OS            *;
**************************;
proc sgplot data=apprating;
	vbar OS;
run;


**************************;
* Validate Year + Product*;
**************************;
proc sgplot data=apprating;
	vbar Date/ group=Product ;
	format Date Year.;
run;

proc means data=apprating;
	class Date Product;
	var Rating;
	format Date year4.;
run;
...
proc sgplot data=casuser.apprating;
	vline Date / response=Rating stat=mean;
	format Date Year4.;
quit;

proc sgplot data=casuser.apprating;
	vline Date / response=Rating stat=mean group=os;
	format Date Year4.;
quit;

proc sgplot data=casuser.apprating;
	vline Date / response=Rating stat=mean group=Server;
	format Date Year4.;
quit;
*/